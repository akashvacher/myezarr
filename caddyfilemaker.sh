...
(basic-auth) {
	basicauth / {
    	...
    	# Password is produced as follows:
	    # $ caddy hash-password -p demo 
        # $2a$14$71ti5vJvBzscHHhxd0FZ0eDQlcRvIpn5/T/onYubdGpU4DfT218XS
        
		demo $2a$14$71ti5vJvBzscHHhxd0FZ0eDQlcRvIpn5/T/onYubdGpU4DfT218XS
	}
}
...
q.${DOMAIN} {
	...
	reverse_proxy localhost:8080 # qbittorrent with Wireguard VPN
}
...
radarr.${DOMAIN} {
	...
	reverse_proxy localhost:7878 # Radarr
}
...
sonarr.${DOMAIN} {
	...
	reverse_proxy localhost:8989 # Sonarr
}
...
prowlarr.${DOMAIN} {
	...
	reverse_proxy localhost:9696 # Prowlarr
}
...
bazarr.${DOMAIN} {
	...
	reverse_proxy localhost:6767 # Bazarr
}
...
fb.${DOMAIN} {
	...
	reverse_proxy localhost:8081 # Filebrowser
}

jellyfin.${DOMAIN} {
	# TLS handled by Caddy by default
	# Routed trafic via a firewall for hiding server IP
	reverse_proxy localhost:8096 # Jellyfin UI
}

dl.${DOMAIN} {
	tls internal # This is because Cloudflare is handling the TLS

	root * $HOME/ezarr/data
	file_server {
		browse $HOME/browse.html # Custom file for browsing to allow only read/download access
	}
}
